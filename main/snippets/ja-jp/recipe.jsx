...
export const SignUpForm = () => {
  ...
  function LoggedInForm({ sampleApp }) {
    ...
    function loadApps() {
      ...
      const seeded = [{ id: "{yourClientId}", name: "デフォルトアプリ" }]
              ...
              <Card className="p-5 hover:shadow-md transition">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-lg">新しいアプリケーションを作成</h2>
                  </div>
              ...
              <Card className="p-5 hover:shadow-md transition">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-lg">既存のアプリケーションと連携</h2>
                  </div>
                  <RightChevron className="w-4 h-4 text-zinc-500" />
                </div>
              </Card>
              ...
              <Card className="p-5 hover:shadow-md transition">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-lg">サンプルアプリケーションを表示</h2>
                  </div>
                  <RightChevron className="w-4 h-4 text-zinc-500" />
                </div>
              </Card>
    ...
    function CreateForm({ onSave, onCancel }) {
      ...
      return (
        <div className="space-y-6">
          <Input id="app-name" label="アプリケーション名" placeholder="マイアプリ" value={name} onChange={setName} />
          <p className="text-sm text-zinc-500">後でアプリケーション設定から変更できます。</p>
    ...
    function IntegrateForm({ apps, selected, onSelect, saved, onSave, onCancel }) {
      ...
      return (
        <div className="space-y-6">
          <div>
            <span className="block text-sm text-zinc-600 dark:text-zinc-300 mb-1">アプリケーションを選択</span>
          ...
          >
            <Input id="callbacks" name="callbacks" label="コールバック URL" placeholder="http://localhost:3000" value={callbacks} onChange={setCallbacks} />
            <Input id="logout" name="allowed_logout_urls" label="ログアウト URL" placeholder="http://localhost:3000" value={logouts} onChange={setLogouts} />
            <Input id="origins" name="web_origins" label="許可される Web オリジン" placeholder="http://localhost:3000" value={origins} onChange={setOrigins} />
    ...
    return (
      <div className="flex flex-col gap-2 items-center h-full">
        ...
        >
          ...
          または{" "}
          <span className="font-semibold text-primary dark:text-white cursor-pointer" onClick={() => console.log("ログイン")}>
            ログイン
          </span>{" "}
          既存のアカウントにログインして、ご自身のテナントと直接連携できます。
        </span>
        <button
...
export const CreateInteractiveApp = ({
  ...
  const CreateApplicationForm = () => {
    ...
    const handleSubmit = async () => {
      ...
      try {
        // 作成 client using the clientStore method
      ...
      <Card className="flex-col items-start p-4 gap-3.75">
        ...
        <div className="w-full flex gap-2">
          <input
            ...
            "
            placeholder={`マイ ${placeholderText} アプリ`}
            value={name}
