...
<Tabs>
  ...
  <Tab title="Management API">
    ...
    **リクエストボディの例**
    ...
    3. レスポンスとして、endpoint は作成されたエンティティを返します。
...
<Tabs>
  ...
  <Tab title="Management API">
    ...
    **Organizations を使用する場合は、追加の設定が必要です**
    ...
    接続が organization に関連付けられており、サードパーティアプリケーションがその organization を通じてユーザーを認証できるようにする場合は、organization の `third_party_client_access` フィールドを別途 `allow` に設定する必要があります。この手順はセルフサービスウィザードでは行われないため、テナント管理者が完了する必要があります。
    ...
    **リクエストボディの例**
    ...
    チケットURLを受け取ったら、そのリンクをカスタマー管理者と共有して、セルフサービスアシスタントへのアクセスを付与してください。アシスタントは SSO 接続の設定を案内します。この体験の詳細については、[セルフサービスアシスタントのエクスペリエンス](/docs/ja-jp/authenticate/enterprise-connections/self-service-enterprise-configuration#self-service-assistant-experience) をご覧ください。
...
<Tabs>
  ...
  <Tab title="Management API">
    ...
    | `enabled_features.sso`                  | **任意**。ブール値。<br /><br />`true` の場合、IT 管理者は SSO 接続を編集できます。`false` に設定すると、接続を変更することなく、プロビジョニングまたはドメイン検証のフローにのみアクセスできるようになります。                                                                                                                                                                                                                                                                                                                                                                                                                 |
    ...
    | `provisioning_config`                   | **任意。** オブジェクト。<br />顧客管理者が SCIM を設定できるかどうかを指定します。接続の作成時に、すべてのユーザープロビジョニング スコープ (`get:users`,`post:users`,`put:users`, `patch:users`,`delete:users`) が含まれていない場合、SCIM は有効になりません。グループ スコープは任意です。[Google Workspace Directory Sync](/docs/ja-jp/authenticate/identity-providers/enterprise-identity-providers/google-directory-sync) を設定するには、[`google_workspace`](https://auth0.com/docs/api/management/v2/self-service-profiles/post-sso-ticket) を使用します。                                                                                             |
